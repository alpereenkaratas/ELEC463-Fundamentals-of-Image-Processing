...
Image Image::createImageforlab3q0(){  // bu fonksiyon LAB 3 soru 0 icin olusturulmustur
  ...
  // istenen ozelliklerde 3x3 goruntu olusturuluyor
  temp.createImage(r, c);
  for (rows = 0; rows < r; rows++){
    for (cols = 0; cols < c; cols++){
        temp(rows,cols) = ctr;  // ilgili piksel degeri goruntuye ataniyor
        ctr++;
    }
  }
  return temp.logTransform();
}


Image Image::moveImageToCenter() { // goruntunun DC bilesenini merkeze almak icin yazilmis fonksiyondur
  Image temp;
  int x,y;
  temp.createImage(nrows, ncols);
  ...
  for(x = 0; x < nrows; x++){ 
      for(y = 0; y < ncols; y++){
          temp(x,y)=(image[x * ncols + y]) * pow(-1, (x+y));  // DC'yi merkeze almak icin gerekli formul
      }
  }
...
Image Image::twoDimFourierTransform(){

  Image temp, F_r1, F_r2, F_i1, F_i2;  // goruntu(temp), satir icin reel(F_r1) ve imajiner(F_i1) kisim, sutun icin reel(F_r2) ve imajiner(F_i2) kisimlar
  int u, v, x, y;
  float max,min;

  // goruntuler olusturuluyor
  temp.createImage(nrows,ncols);

  F_r1.createImage(nrows, ncols);
  F_i1.createImage(nrows, ncols);
  F_r2.createImage(nrows, ncols);
  F_i2.createImage(nrows, ncols);

  // satirin fourier donusumu icin asagidaki blok kuruldu
  for (u = 0; u < nrows; u++){
    for (y = 0; y < ncols; y++){
      for(x = 0; x < nrows; x++){          
        F_r1(u,y) = F_r1(u,y) + image[x * ncols + y] * cos(2*M_PI*x*u/nrows); // 2d fourier donusum formulu (real)
        F_i1(u,y) = F_i1(u,y) - image[x * ncols + y] * sin(2*M_PI*x*u/nrows); // 2d fourier donusum formulu (imag)
      }
    }
  }

  // sutunun fourier donusumu icin asagidaki blok kuruldu
  for (u = 0; u < nrows; u++){
    for (v = 0; v < ncols; v++){
      for(y = 0; y < ncols; y++){           
        F_r2(u,v) = F_r2(u,v) + F_r1(u,y) * cos(2*M_PI*v*y/ncols) + F_i1(u,y) * sin(2*M_PI*v*y/ncols); // 2d fourier donusum formulu (real)
        F_i2(u,v) = F_i2(u,v) + F_i1(u,y) * cos(2*M_PI*v*y/ncols) - F_r1(u,y) * sin(2*M_PI*v*y/ncols); // 2d fourier donusum formulu (imag)
      }
    }
  }
  
  // real ve imag kisimlarin karelerinin karekoku alinarak goruntu elde edilir
  for (x = 0; x < nrows; x++){
    for (y = 0; y < ncols; y++){
      temp(x,y) = sqrt( pow(F_r2(x,y),2) + pow(F_i2(x,y),2) ); 
    }
  }

  // olcekleme icin maks ve min degerleri bulundu
  max = temp.getMaximum();
  min = temp.getMinimum();
  
  for (x = 0; x < nrows; x++){
    for (y = 0; y < ncols; y++){
      temp(x,y) = (temp(x,y) - min) * 255 / (max - min); // olcekleme yapiliyor
    }
  }
...
Image Image::inverseTwoDimFourierTransform(){
  ...
  return temp/(nrows*ncols);

}

Image Image::sobelFilterXaxis(int m, int n){

  Image temp;
  temp.createImage(nrows, ncols);

  int mask[3][3] = {-1, -2, -1, 0, 0, 0, 1, 2, 1};   // 3x3'luk bir sobel filtresi (x axis) icin maskenin ici ilgili degerlerle dolduruldu
  int pixVal = 0;
...
for (int rows = 0; rows < nrows; rows++){
  for (int cols = 0; cols < ncols; cols++){
    ...
    temp(rows,cols) = abs(pixVal);  // formul geregi (derivatives for enhancement) x eksenindeki gradyan degerinin mutlagi aliniyor
    pixVal = 0;   // goruntunun sonraki pikselleri icin pixVal degeri sifirlaniyor
  }
} 
...
Image Image::sobelFilterYaxis(int m, int n){

  Image temp;
  temp.createImage(nrows, ncols);

  int mask[3][3] = {-1, 0, 1, -2, 0, 2, -1, 0, 1};   // 3x3'luk bir sobel filtresi (y axis) icin maskenin ici ilgili degerlerle dolduruldu
  int pixVal = 0;
...
for (int rows = 0; rows < nrows; rows++){
  for (int cols = 0; cols < ncols; cols++){
    ...
    temp(rows,cols) = abs(pixVal);  // formul geregi (derivatives for enhancement) y eksenindeki gradyan degerinin mutlagi aliniyor
    pixVal = 0;   // goruntunun sonraki pikselleri icin pixVal degeri sifirlaniyor
  }
} 
...
Image Image::lab4Q2filter(){
  Image temp;
  temp.createImage(nrows, ncols); 

    for(int u = 0; u < nrows; u++){ 
        for(int v = 0; v < ncols; v++){
            temp(u,v) = -4*pow(M_PI,2)*(pow(u,2)+pow(v,2));  // filtrenin formulu
        }
    }
  return temp;
}

Image Image::lab4Q2shiftedFilter() { 
  Image temp;
  temp.createImage(nrows, ncols); 

    for(int u = 0; u < nrows; u++){ 
        for(int v=0; v < ncols; v++){
            temp(u,v) = -4*pow(M_PI,2)*(pow((u-nrows/2),2)+pow((v-ncols/2),2)); // shift edilmis filtrenin formulu
        }
    }
  return temp;
}
